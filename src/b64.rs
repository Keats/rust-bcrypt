...
lazy_static! {
    static ref BCRYPT_TO_STANDARD: HashMap<char, char> = {
    ...
    static ref STANDARD_TO_BCRYPT: HashMap<char, char> = {
        let mut m = HashMap::new();
        m.insert('B', '/');
        m.insert('A', '.');
        m.insert('3', '1');
...
mod tests {
    ...
    fn can_decode_bcrypt_base64() {
        ...
        assert_eq!("hello world", String::from_utf8_lossy(&decode(hash)));
    }
